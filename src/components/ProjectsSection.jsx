...
import { ArrowRight, Rocket, Code2, Info, AlertTriangle, TrendingUp, X } from 'lucide-react'
...
const PROJECTS = [
  {
    ...
    cardImage: 'https://lh3.googleusercontent.com/aida-public/AB6AXuD9kllg2U_bldFO3Hi3yLUOxtNbSDc_SuWoF-sLk-glsxzI3UX4cHu0BNF6nyMo4EQ4Mj0oAAzWMddmDPSBVvwELNlsu-9IWdnuKNn_TgtJj72n0ZaWMwRhgdDlkOVD2bJj4xKrqKMossFGFeEVs-FBz316fmEhb4ErMJJQeLj_OZTKZbKST8e_uG1aKrla-_gkokqKNUy0gdJu4ZMh4b--C1nCTGMv-S55UFQFVlU33OpHHtZzZbJQDu1mYoEk3KuxyyotzOhn0mo',
  ...
  },
  {
    ...
    tag: 'React Native',
    tagColor: 'text-white bg-white/10 border border-white/20',
    cardImage: 'https://lh3.googleusercontent.com/aida-public/AB6AXuBU3OEGrno2503OVpuryT7QFpH61W7nd5b9FFhlGJpc1I-YtK9AjCjZFV1AMaoWgAl-bi8OQZ0xkILrlihXb_cHTRwm2BT1i8eqF7xaisinrazl8rx4_lkJj841Fm9euAMEs6uVz-jcpGy0Ccp4XjL_tf0LzPLaWQIeBcnaUB5b70lGEs9BVWRx9FqZ_rt1wis7j90hKzhBuaO9GoCX9YFIae8VSQ0DjzFwnpO2zqr5mZvIxjzK_ijGUs_hXjybbwVnHgUfhQqqrZo',
    ...
    liveUrl: '#', repoUrl: '#',
    overview: 'A full-featured mobile banking app built for security-first users. Biometric login, encrypted local storage, and real-time push notifications keep users informed and protected at every step.',
    challenges: 'Implementing biometric authentication across both iOS and Android with a unified API was tricky. Used expo-local-authentication with a custom fallback PIN flow to ensure 100% device coverage.',
    improvements: 'Planning to add open-banking API integrations so users can aggregate accounts from multiple banks, and introduce a budgeting AI that learns from spending patterns.',
  },
  {
    ...
    tag: 'Vue.js',
    tagColor: 'text-white bg-white/10 border border-white/20',
    cardImage: 'https://lh3.googleusercontent.com/aida-public/AB6AXuArYzOUIgk6kC5DWIpovqVPQs5uSg0uPwL4IGwlXTqkLAZrgTIvTG7zlD_Oo4jDTb2vGHuadpHlEq827F3g9lpDqQQktaNvYqwsXxN3wgybxHDYTh-wOXjPJTcDNNpYQ8PZksoNYGoipX98N_X5Dgm9-rJBnIFOSD-fKxnloZgTOSW0XqOQgQCSujFHQiO1WqWY4aMLkhIqnMJfiRS0s_V6AjqA3A9anOB5ugH1TxXNMwOVwV3sv6HW8mX2pSImAt38Dnf7avhZVSo',
    ...
    liveUrl: '#', repoUrl: '#',
    overview: 'A real-time collaborative task manager built for distributed teams. Drag-and-drop kanban boards, role-based permissions, and live presence indicators make async work feel synchronous.',
    challenges: 'Syncing drag-and-drop state across multiple clients in real time without conflicts required a CRDT-inspired approach using Firebase Realtime Database with optimistic local updates.',
    improvements: 'Looking to add Gantt chart views, time-tracking integrations with Toggl, and an AI assistant that auto-prioritizes tasks based on deadlines and team velocity.',
  },
]
...
function ProjectCard({ project, index, isSelected, onSelect }) {
  return (
    <article
      ...
      className={`group relative flex flex-col bg-[#151921] rounded-2xl overflow-hidden border transition-all duration-300 cursor-pointer hover:-translate-y-1
        ${isSelected
          ? 'border-white shadow-[0_0_20px_rgba(255,255,255,0.1)]'
          : 'border-gray-800 hover:border-white/40 hover:shadow-[0_0_20px_rgba(255,255,255,0.05)]'
        }`}
    >
      {/* Image */}
...
function DetailPanel({ project, onClose }) {
  const details = [
    { Icon: Info,          iconBg: 'bg-white/10', iconColor: 'text-white',      label: 'Overview',             text: project.overview     },
    { Icon: AlertTriangle, iconBg: 'bg-gray-700',  iconColor: 'text-gray-300',  label: 'Challenges Faced',     text: project.challenges   },
    { Icon: TrendingUp,    iconBg: 'bg-gray-700',  iconColor: 'text-gray-300',  label: 'Future Improvements',  text: project.improvements },
  ]
      ...
      <div className="w-full lg:w-5/12 bg-[#0f1419] p-6 lg:p-8 flex flex-col gap-6 border-b lg:border-b-0 lg:border-r border-gray-800">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-3">
            <a href={project.liveUrl} className="flex-1 flex items-center justify-center gap-2 bg-white hover:bg-gray-200 text-black py-3 px-4 rounded-lg font-medium transition-colors min-w-[140px]">
              <Rocket size={18} /> Live Demo
            </a>
            <a href={project.repoUrl} className="flex-1 flex items-center justify-center gap-2 bg-gray-800 hover:bg-gray-700 text-white py-3 px-4 rounded-lg font-medium transition-colors min-w-[140px]">
        ...
        <div className="flex flex-col gap-3 mt-auto">
          ...
          <div className="flex flex-wrap gap-2">
            {project.stack.map((tech) => (
              <span key={tech} className="px-3 py-1 rounded-md bg-white/10 text-white text-xs font-mono border border-white/20">
                {tech}
              </span>
            ))}
          </div>
      ...
      <div className="w-full lg:w-7/12 p-6 lg:p-10 flex flex-col gap-8">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-gray-400 text-lg font-medium">{project.subtitle}</p>
          </div>
          <button
            onClick={onClose}
